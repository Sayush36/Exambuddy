...
export default function AdminNotesPage() {
    ...
    const handleEdit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setIsEditOpen(false);
                setNotes(notes.map(n => n.id === selectedNote.id ? { ...n, title: formData.title, branch: formData.branch, is_premium: formData.is_premium } : n));
    ...
    const filteredNotes = notes.filter(n => {
        ...
        const matchesBranch = filterCourse === "ALL" || n.branch.includes(filterCourse);
        const matchesCategory = filterCategory === "ALL" || n.category === filterCategory;
        const matchesStatus = filterStatus === "ALL" || n.status === filterStatus;
        const matchesSemester = filterSemester === "ALL" || n.semester.toString() === filterSemester;
        return matchesSearch && matchesBranch && matchesCategory && matchesStatus && matchesSemester;
    });
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    ...
                    >
                        ...
                        <option value="SESSIONAL_PAPER">Sessional Papers</option>
                        <option value="UNIVERSITY_PAPER">University Papers</option>
                    </select>
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent>
                    ...
                    <form onSubmit={handleEdit} className="space-y-4">
                        ...
                        <div>
                            <Label htmlFor="edit-branch">Branches (comma-separated)</Label>
                            <Input id="edit-branch" placeholder="e.g. CS, IT" value={formData.branch} onChange={e => setFormData({ ...formData, branch: e.target.value })} required />
                        </div>
                        <div className="flex items-center gap-2">
                            <input
                                type="checkbox"
